...
export function TeacherDetail(props) {
  const [form] = Form.useForm();
  console.log("Recieved props", props)

  const update_teacher_profile = (requestdata )=>{
  ...
  const onFinish = (values) => {
    ...
    if(myuserobject&&myuserobject.teacher){
      if (values && values.fields) {
        myteacherdata= [...values.fields,...myuserobject.teacher];
        const requestdata = { "teacher":myteacherdata }
        update_teacher_profile(requestdata)
      }
    } else {
      myteacherdata=[...values.fields]
      const requestdata = { "teacher":myteacherdata }
      update_teacher_profile(requestdata)
    }
  }
                  ...
                  <div key={field.key}>
                    ...
                    >
                       <Select>
                        ...
                        <Select.Option value="both">दोनों</Select.Option>
