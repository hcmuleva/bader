...
export function ShowRoomDetail(props) {
  const [form] = Form.useForm();
  console.log("Recieved props", props)

  const update_showroom_profile = (requestdata )=>{
  ...
  const onFinish = (values) => {
    ...
    if(myuserobject&&myuserobject.showroom){
      if (values && values.fields) {
        myshowroomdata= [...values.fields,...myuserobject.showroom];
        const requestdata = { "showroom":myshowroomdata }
        update_showroom_profile(requestdata)
      }
    } else {
      myshowroomdata=[...values.fields]
      const requestdata = { "showroom":myshowroomdata }
      update_showroom_profile(requestdata)
    }
  }
            ...
            return (
              <div>
                {fields.map((field, index) => (
                  <div key={field.key}>
                    <Divider>शोरूम की जानकारी- {index + 1}</Divider>
                    <Form.Item
                      label="शोरूम का नाम"
                      name={[index, "showroom_name"]}
                    >
                    <Input/>
                    </Form.Item>
                    <Form.Item
                      label="शोरूम टाइप"
                      name={[index, "showroom_type"]}
                    >
                      <Input />
                    </Form.Item>
                    <Form.Item
