...
const HeaderComponent = ({ checked, onChangeSwitch, selectedRole, setSelectedRole, handleLogout }) => {
  ...
  console.log("fromStorage object ", fromStorage)
  const user = fromStorage ? JSON.parse(fromStorage) : undefined;
  //console.log("USER Obje before role change", user, "  other value", user.user_avatar)
  ...
  //console.log("USER OBJECT ",user.user_avatar)
  const userProfile = user&&user.user_avatar !== '' ? user.user_avatar : undefined ;
  console.log("Recieved userProfile ", userProfile)
  const dropdownMenu = window.innerWidth <= 768;

  const renderMenu = (
    <Menu
      selectedKeys={[current]}
      className={`header-menu vertical ${dropdownMenu ? 'has-dropdown' : ''}`}
